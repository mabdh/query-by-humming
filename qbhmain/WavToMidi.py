...
class WavToMidi:
    ...
    def getEnergy(self,streamData):
    	sumRes = np.sum(abs(streamData)**2.0)
    	return sumRes/len(streamData)

    def fundamental_freq_fft(self,signal,sampleRate):
        n = len(signal)
        # print(n)
    ...
    def show_spectrogram(self, x, fs):
    	plt.figure(12)
    ...
    def convert(self, filename, hflag):
        if hflag==0:
            stream = self.open("wavFile/" + filename + ".wav")
        else:
            stream = self.open("hummFile/" + filename + ".wav")
            
        freqArray, tick_ms = self.fundamental_freq_fft(stream,self.sampleRate)
